...
foreach ($posts as $post) {
    ...
    foreach ($acf_feature_categories as $acf_feature_category) {
        ...
        foreach ($acf_feature_category['feature'] as $feature) {
			...
			$feature_tooltip = trim($feature['feature_tooltip']);
      ...
      <?php foreach ($feature_categories as $feature_category) : ?>
        ...
        <?php foreach ($feature_category['features'] as $feature) : ?>
          <tr class="compare-body__feature">
            <th><span><?php echo $feature['description']; ?></span> <?php if (!empty($feature['tooltip'])) { ?><span class="btn-info-icon" style="float: right; font-size: 1em;" data-toggle="tooltip" data-placement="right" title="<?php echo $feature['tooltip']; ?>"><img src="<?php echo get_stylesheet_directory_uri(); ?>/assets/images/icon_info_grey.svg" alt=""></span><?php } ?></th>
                ...
                <div class="compare-features-mobile__feature-wrap">
                  <div class="compare-features-mobile__feature-row">
					...
					if ( !empty( $feature['tooltip']) ) { 
						$tooltip = '<span class="btn-info-icon" style="float: right; font-size: 1em;" data-toggle="tooltip" data-placement="right" title="' . $feature['tooltip'] . '"><img src="' . get_stylesheet_directory_uri() . '/assets/images/icon_info_grey.svg" alt=""></span>';
					}					
					?>
                      <div class="compare-features-mobile__feature">
                        ...
                        switch ($feature['editions'][$index]) {
                          case 'Yes':
                            echo '<table><tr><td><img src="'. get_template_directory_uri() .'/assets/images/check.png"></td><td>' . $feature['description'] . $tooltip . '</td></tr></table>';
                            break;
                          case 'No':
                            echo '<table><tr><td><img src="'. get_template_directory_uri() .'/assets/images/close.png"></td><td>' . $feature['description'] . $tooltip . '</td></tr></table>';
                            break;
                          default:
                            echo $feature['editions'][$index] . ' ' . $feature['description'] . $tooltip;
                            break;
                        } ?>
                      </div>
                    <?php endforeach; ?>
