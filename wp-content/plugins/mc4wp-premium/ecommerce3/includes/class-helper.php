...
class MC4WP_Ecommerce_Helper {
	...
	*/
	private function get_product_query( $select = 'p.*' , $tracked_only = false) {
		$query = sprintf( "SELECT %s
			FROM {$this->db->postmeta} pm RIGHT JOIN {$this->db->posts} p ON p.id = pm.post_id
	...
	*/
	private function get_order_query( $select = 'p.*', $tracked_only = false ) {
		...
		$query = "SELECT %s FROM {$this->db->postmeta} pm RIGHT JOIN {$this->db->posts} p ON p.id = pm.post_id
		...
		if( $tracked_only ) {
			...
			$query .= " AND ( pm.meta_key = '_billing_email'
				OR pm.meta_key = 'billing_email'
				OR pm.meta_key = '_customer_user' )
				AND pm.meta_value != ''";
		}
