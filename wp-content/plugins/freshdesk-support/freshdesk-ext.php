...
function fd_login_redirect( $url, $request, $user ) {
	...
	if ( isset( $_REQUEST['wp-submit'] ) && $_REQUEST['wp-submit'] == "Log In" && is_a( $user, 'WP_User' ) && $redirect_url ) {
		...
		$ssl_url = freshdesk_sso_login_url( $redirect_url, $user_name, $user_email, $secret);
...
function get_redirect_url($host_url) {
	...
	$domains = explode( ",", $freshdesk_options['freshdesk_cname'] );
	...
	if ( 'wp-login.php' == $pagenow ){
		...
		if ( $_GET['action'] == 'freshdesk-login' ) {
			...
			if ( is_user_logged_in() ) {
				...
				$url = freshdesk_sso_login_url( get_redirect_url($domain), $user_name, $user_email, $secret);
				header( 'Location: '.$url ) ;
				die();
			}
		...
		if ( $_GET['action'] == 'freshdesk-logout' ) {
			wp_logout();
			header( 'Location: '.get_redirect_url($domain) );
			die();
		}
	}
...
function freshdesk_sso_login_url($domain, $user_name, $email, $secret){
	$time = time();
