...
class Hpj_CMLabs_License_Controller extends Hpj_CMLabs_Controller {
    ...
    private function decorateLicense($fulfillment) {
        # Node-locked: send as-is.
        # Floating: must prepend the first to lines (SERVER + ISV)
        $license = $fulfillment->license;
        if (strpos($license , "uncounted") === false) {
            # "counted" means floating.  Add missing lines.
            $line1 = sprintf("SERVER %s %s 5053\n", $fulfillment->hostname, $fulfillment->license_hostid);
            $line2 = "ISV cmlabs\n";
            $license = $line1 . $line2 . $license;
        }
        return $license;
    }
    ...
    public function activationDownloadAction() {
        ...
        if (is_user_logged_in()) {
            ...
            if (!empty($id)) {
                ...
                if (!empty($data)) {
                    $datas['license'] = $this->decorateLicense($data);
                }  
            }
        }
        return array(
            ...
            if (!empty($akey) && !empty($hostId)) {
                ...
                if (!empty($activationKey)) {
                    ...
                    if (!empty($data)) {
                        ...
                        echo $this->decorateLicense($data);
                        exit();
                    } else {
